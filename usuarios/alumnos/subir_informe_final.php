...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['informe']) && $_FILES['informe']['error'] === 0) {
        ...
        $nombre_archivo = strtolower($apellido) . '_' . strtolower($nombre) . '_informe' . $id_informe . '_final.pdf';
        ...
        // Prepared statement.
        $stmt = $mysqli->prepare("INSERT INTO informes (dni_alumno, id_informe, original, nombre_archivo, ruta_archivo, fecha_subida, estado, final) VALUES (?, ?, 1, ?, ?, ?, ?, 1)");
        $stmt->bind_param("sissss", $_SESSION['dni'], $id_informe, $nombre_archivo, $ruta_archivo, $fecha_subida, $estado);
        $stmt->execute();

        // Establecer mensaje de éxito y cerrar la conexión a la base de datos.
        $_SESSION['mensaje_exito'] = "Se ha registrado el informe final con éxito.";
        $mysqli->close();
    } else {
